 ...
 */
 function verifyPresenceOfRegisterCredentials(userModel, request, response) {
    if(userModel.lastname == null || userModel.firstname == null || 
        userModel.email == null || userModel.password == null ||
        userModel.url == null) {
            response.statusCode = 400;
            request.statusCodeMessage = "Bad Request";
 ...
 * @returns 1, daca am primit destule informatii; 0, altfel
 */
 function verifyPresenceOfChangeURLCredentials(previousURL, userModel, request, response) {
    if(userModel.url == null) {
        response.statusCode = 400;
        request.statusCodeMessage = "Bad Request";
        request.errorMessage = "Request-ul de tip PUT primit la pagina \"/profile/change_url\" nu este unul valid!";
        errorRoute(request, response);
        return 0;
    }
 ...
 */
 function verifyPresenceOfChangePasswordCredentials(oldPassword, userModel, request, response) {
    if(userModel.password == null) {
        response.statusCode = 400;
        request.statusCodeMessage = "Bad Request";
        request.errorMessage = "Request-ul de tip PUT primit la pagina \"/profile/change_password\" nu este unul valid!";
        errorRoute(request, response);
        return 0;
    }
